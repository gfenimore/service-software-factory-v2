...
class TestQualityDashboard {
  ...
  generateDashboard() {
    ...
    if (this.metrics.components.length > 0) {
      ...
      this.metrics.summary.averageQualityScore = Math.round(
        totalScore / this.metrics.components.length
      )
    }
...
═══════════════════════════════════════════════════`
    ...
    const untested = metrics.components.filter((c) => !c.hasTests).slice(0, 5)
