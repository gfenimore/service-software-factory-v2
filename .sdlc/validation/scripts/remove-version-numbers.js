...
class VersionRemover {
  ...
  generateReport(references) {
    ...
    report.push(`- Files renamed: ${this.changes.filter(c => c.oldFilename !== c.newFilename).length}`);
    ...
    if (agentChanges.length > 0) {
      ...
      agentChanges.forEach(change => {
        ...
        report.push(`- ${change.oldFilename} ${status} ${change.newFilename} (Version: ${change.version})`);
      });
      report.push('');
    }
  ...
  run() {
    ...
    agentFiles.forEach(file => {
      ...
      if (result) {
        ...
        console.log(`${action} ${result.oldFilename}${renamed ? ' → ' + result.newFilename : ''} (v${result.version})`);
      }
    });
